...
id 어트리뷰트 값: id 어트리뷰트 값을 지정하여 일치하는 요소를 선택한다. id 어트리뷰트 값은 중복될 수 없는 유일한 값이다. -> 건들지 않아야 하는 셀렉터이다.

.class : class 어트리뷰트 값을 지정하여 일치하는 요소를 선택한다. 

class 어트리뷰트 값은 중복될 수 있다. -> 동시 적용이 가능하다. 

계속 사용하는 셀렉터임. 이 수업에서는 이것만 하기로함.
...
백분율 단위의 상대 단위(비율)이다. 요소에 지정된 사이즈에 상대적인 사이즈를 설정한다.
...
> 반응형 web design 이기 때문이다. 비율이 중요해지는 시대가 왔다.
...
W3C css3-color를 참고,  Color keywords 도 있다.
...
2. 높이(height)는 컨텐트(내용)의 세로와 동일하다.
